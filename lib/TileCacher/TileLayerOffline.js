define(['dojo/_base/declare', 'dojo/Deferred', '/lib/Leaflet/leaflet-src.js'], function(declare, Deferred, L) {
	...
	return declare([L.TileLayer], {
		...
		/**
		 * Handles tile loading event by adding tile to store.
		 * @param {Image} tile
		 * @param {Point} tilePoint
		 */
		...
		addToCache: function(tile, key) {
			var img;
			// note: setting this attribute on an image with a data uri will prevent display and loading of the image
			// -> do this only for loading from url
			// TODO: can be moved back to _loadTile() as soon as https://bugzilla.mozilla.org/show_bug.cgi?id=1109693 is fixed
