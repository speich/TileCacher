define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/topic', 'lib/Leaflet/leaflet.js'], function(declare, lang, topic, L) {
	...
	return declare(null, {
		...
		_saveNext: function(queue, zoom) {
			var self = this, key, tilePoint,
				layer = this.layer;

			tilePoint = queue.shift();

			if (tilePoint) {
				tilePoint = this.adjustTilePoint(tilePoint, zoom);
				key = layer.getKey(tilePoint, layer.options.zoomOffset);

				// try to load tile from db in case it was already cached previously
				layer.store.get(layer.name, key)
					.then(function(value) {
						// tile not found ?
						if (!value) {
		...
		addToCache: function(tile) {
			var layer = this.layer,
				key = tile._storageKey;	// get key before converting img to data

			tile = layer._imageToDataUri(tile);
			layer.store.add(layer.name, key, tile).then(function() {
